...
def iou_binary(preds, labels, EMPTY=1., ignore=255, per_image=True):
    ...
    for pred, label in zip(preds, labels):
        ...
        ious.append(iou)
    iou = sum(ious) / len(ious)  # mean accross images if per_image

    return 100 * iou



def calcF1(preds, labels, ignore=255, per_image=True):
    """
    IoU for foreground class
    binary: 1 foreground, 0 background
    """
    ...
    for pred, label in zip(preds, labels):
        ...
        if ignore != None:
            FP = ((label == 0) & ((pred == 1) & (label != ignore))).sum()
            FN = ((label == 1) & ((pred == 0) & (label != ignore))).sum()
            # TN = ((label == 0) & ((pred == 0) & (label != ignore))).sum()
        ...
        if (precision + recall) != 0:
            ...
            f1score.append(0.0)
    mean_f1 = sum(f1score) / len(f1score)  # mean accross images if per_image
    return 100*mean_f1


def iou(preds, labels, C, EMPTY=1., ignore=255, per_image=False):
    """
